...
class BannerManager extends Manager {
    ...
    public function getBanners() {
        ...
        FROM projet_4_banners ORDER BY display_order LIMIT 5'); 
    ...
    public function getBanner($banner_id) {
        ...
        DATE_FORMAT(creation_date, \'%d/%m/%Y à %Hh%i\') AS creation_date, DATE_FORMAT(modification_date, \'%d/%m/%Y à %Hh%i\') AS modification_date 
        FROM projet_4_banners WHERE id = ?');
    ...
    public function createBanner(Banner $banner) {
        $db = $this->MySQLConnect();
        $req = $db->prepare('INSERT INTO projet_4_banners(display_order, title, caption, image, button_title, button_link, creation_date, modification_date)
    ...
    public function editBanner(Banner $banner) {
        ...
        if (empty($banner->image())) {
            ...
            $req= $db->prepare('UPDATE projet_4_banners SET title = :title, caption = :caption, button_title = :button_title, button_link = :button_link, image = :image, modification_date = NOW()
            WHERE id = :id');
    ...
    public function deleteBanner($banner_id) {
        $db = $this->MySQLConnect();
        $req = $db->prepare('DELETE FROM projet_4_banners WHERE id = ?');
